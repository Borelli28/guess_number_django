...
def index(request):
    ...
    # If user input is a word it will break the program and will need to reset the guess number manually by uncommenting the line below
    # request.session['guess'] = 666

    # if user guess is not empty
    if 'guess' in request.session and 'number' in request.session:

        # create a display variable in session
        ...
        if request.session['number'] < int(request.session['guess']):
            ...
            request.session['display'] = "Too High!"
        ...
        # User guess is less than the random number
            ...
            request.session['display'] = "Too Low!"
        ...
        # if user guess the number then generate a new random number
            ...
            request.session['display'] = f"You got it! {request.session['number']} was the number... CONGRATS!"
